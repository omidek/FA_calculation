...
% projection of the signed differences onto the DA direction, this produces a DA score for each face
DA_scores = signed_diff*DA_direction;

% calculating the vector of projections for each face
DA_projections = repmat(DA_scores,1,size(signed_diff,2)).*repmat(DA_direction',size(signed_diff,1),1);
